...
track_path(const std::vector<std::vector<Cell>> &cell_details,
	...
	while (!(i == src.first && j == src.second)) {
		...
		if (!is_valid(temp_i, temp_j, 32, 32)) {
...
plan_path(const std::vector<std::vector<int>> &grid,
	...
	std::vector<std::vector<bool>> closed_list(32,
											   std::vector<bool>(32, false));

	std::vector<std::vector<Cell>> cell_details(32,
												std::vector<Cell>(32, Cell()));
